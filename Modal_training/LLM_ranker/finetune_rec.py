...
def train(
    ...
    def preprocess_logits_for_metrics(logits, labels):
        ...
        """
        labels_index = torch.argwhere(torch.bitwise_or(labels == first_id, labels == second_id))
        gold = torch.where(labels[labels_index[:, 0], labels_index[:, 1]] == second_id, 0, 1)
        labels_index[: , 1] = labels_index[: , 1] - 1
        logits = logits.softmax(dim=-1)
        logits = torch.softmax(logits[labels_index[:, 0], labels_index[:, 1]][:,[second_id, first_id]], dim = -1)
